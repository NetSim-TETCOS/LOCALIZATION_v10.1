...
_declspec(dllexport) int fn_NetSim_Zigbee_Run(void** var)
{
	...
	case PHYSICAL_IN_EVENT: 
		{
			...
			if(ZIGBEE_ERRORPATHLOSS((int)dErrorRange,pstruEventDetails->dPacketSize,dBER))
			{
	...
	case TIMER_EVENT:
		{
			...
			case UPDATE_MEDIUM:
				{
					...
					for(nLoop=1; nLoop<=NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.nConnectedDeviceCount; nLoop++)
					{
						NETSIM_ID nDeviceId= NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.anDevIds[nLoop-1];
			...
			case CFP_END:
				{
					NETSIM_ID nDeviceId,nLink_Id, nConnectionID, nConnectionPortID,nLoop;
					nLink_Id = fn_NetSim_Stack_GetConnectedDevice(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId,&nConnectionID,&nConnectionPortID);
					for(nLoop=1; nLoop<=NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.nConnectedDeviceCount; nLoop++)
					{
...
int fn_NetSim_Zigbee_ChannelFormation(CHANNELS** pstruChannelList,NetSim_EVENTDETAILS* pstruEventDetails)
{
	*pstruChannelList = calloc(1,sizeof(CHANNELS));
	...
	(*pstruChannelList)->dFrequency = WSN_PHY(1)->dFrequencyBand_MHz;
...
{
	is_emulated_packet(pstruEventDetails->pPacket);
	fn_NetSim_Packet_FreePacket(pstruEventDetails->pPacket);
	return 0;
}
